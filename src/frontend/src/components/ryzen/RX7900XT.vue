...
const fetchData = () => {
  fetch("/api/product/search/RX 7900XT")
      .then(response => response.json())
      .then(data => {
        // Filter the data to only include products with '7600' but exclude '7600XT'
        products.value = data.filter(product => product.productName.includes("7900XT") && !product.productName.includes("7900XTX"));
...
<template>
  <div class="products-container">
    <h1>AMD Radeon RX 7900XT Graphics Cards</h1>
    <p>Check out the latest AMD Radeon RX 7900XT products:</p>
