...
const fetchData = () => {
  fetch("/api/product/search/RX 7700XT")
      .then(response => response.json())
      .then(data => {
        // Filter the data to only include products with '7600' but exclude '7600XT'
        products.value = data.filter(product => product.productName.includes("7700XT") && !product.productName.includes("7600XT"));
...
<template>
  <div class="products-container">
    <h1>AMD Radeon RX 7700XT Graphics Cards</h1>
    <p>Check out the latest AMD Radeon RX 7700XT products:</p>
